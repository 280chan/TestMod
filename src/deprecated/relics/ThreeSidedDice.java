 ...
 * @deprecated
 */
public class ThreeSidedDice extends CustomRelic implements MiscMethods {
	
	public static final String ID = "ThreeSidedDice";//遗物Id，添加遗物、替换遗物时填写该id而不是遗物类名。
	public static final String IMG = "resources/images/ThreeSidedDice.png";//遗物图片路径
	
	public static final String DESCRIPTION = "每当一张牌被打出时，有 #b1/3 的概率将其消耗，有 #b1/3 的概率回复其消费的能量，有 #b1/3 的概率额外打出一次。";//遗物效果的文本描叙。
	...
	private void exhaust(UseCardAction action) {
	...
	public void onUseCard(final AbstractCard card, final UseCardAction action) {
		if (!card.purgeOnUse) {
			flash();
			currentSituation = this.getSituation(getRoll());
	...
	private int getRoll() {
	    ...
	    return roll += AbstractDungeon.cardBlizzRandomizer;
	}
	
	private Situation getSituation(int roll) {
	    if (roll < 33) {
	    	return Situation.EXHAUST;
	    }
	    if (roll < 66) {
	    	return Situation.REGAIN;
	    }
	    if (roll < 99) {
	    	return Situation.DUALPLAY;
	    }
	    return getSituation(getRoll());
	}
