...
public class InfectionPower extends AbstractPower{
	
	public static final String POWER_ID = "InfectionPower";//能力的ID，判断有无能力、能力层数时填写该Id而不是类名。
	public static final String NAME = "易传染";//能力的名称。
	public static final String IMG = TestMod.powerIMGPath(POWER_ID);
	public static final String DESCRIPTION = "受到的攻击降为 #b1 ，但获得相同点数的中毒层数。";//不需要调用变量的文本描叙，例如钢笔尖（PenNibPower）。
	public static final String[] DESCRIPTIONS = {"","",""};//需要调用变量的文本描叙，例如力量（Strength）、敏捷（Dexterity）等。
	//以上两种文本描叙只需写一个，更新文本方法在第36行。
	
	private float tempDamage;
	
	public InfectionPower(AbstractCreature owner) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
		this.name = NAME;
		this.ID = POWER_ID;
		this.owner = owner;
		this.amount = -1;
    ...
    public int onAttacked(final DamageInfo info, final int damageAmount) {//参数：info-伤害信息，damageAmount-伤害数值
    	if (info.type == DamageType.NORMAL) {
        	...
        	if (temp > 0) {
        		...
        		AbstractDungeon.actionManager.addToTop(new ApplyPowerAction(owner, p, new PoisonPower(owner, p, temp), temp));
        		if (damageAmount > 0) {
        			return 1;
        		}
        	}
